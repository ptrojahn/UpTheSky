...
void updateLock(Entity* lock) {
	float offset = *(float*)&dynamic_cast<ShopLayerGraphics*>(lock->getLayer())->skinChooser->getComponent<UniformsComponent>()->uniforms[1].data[0];
...
void updatePrice(Entity* price) {
	updateLock(price);
	float offset = *(float*)&dynamic_cast<ShopLayerGraphics*>(price->getLayer())->skinChooser->getComponent<UniformsComponent>()->uniforms[1].data[0];
...
void updateColorChooser(Entity* colorChooser) {
	for (Entity* entity : colorChooser->getLayer()->getEngine()->getEntities()){
		ColorChooserComponent* component = entity->getComponent<ColorChooserComponent>();
		if (component){
			*component->colorOffset = *(float*)&entity->getComponent<UniformsComponent>()->uniforms[1].data[0] * -2 + 0.5f;
